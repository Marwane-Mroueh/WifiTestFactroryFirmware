...
bool end_asercom2 = false;

void set_end_asercom2(bool val)
{
    end_asercom2 = val;
    return;
}
bool get_end_asercom2(void)
{
    return end_asercom2;
}

#define uart1_send_static_text(msg)              \
    do                                           \
    {                                            \
...
#define uart2_send_static_text(msg)              \
    do                                           \
    {                                            \
        e_send_uart2_char(msg, sizeof(msg) - 1); \
        while (e_uart2_sending())                \
            ;                                    \
    } while (0)
#define uart2_send_text(msg)                 \
    do                                       \
    {                                        \
...
int run_asercom2(void)
{
    ...
    { // the I2C must remain disabled when using the gumstix extension
        e_i2cp_init();
    }
#endif
            ...
            if (i != 0)
            {
                if (gumstix_connected == 0)
                {
                    if (wait_cam)
                    {
                        ...
                        memcpy(&buffer[cam_start_index], cam_get_last_image_ptr(), cam_size);
                    }
                }
            ...
            case 'J': // set camera parameter see also cam library
                /*if (gumstix_connected == 0 && get_object_found() == 0)
                {
                    ...
                    if (cam_mode == GREY_SCALE_MODE)
                        ...
                        cam_size = cam_width * cam_heigth * 2;
                    if (cam_size > IMAGE_MAX_SIZE)
                    { // if desired settings too demanding set to a reasonable default
                        cam_mode = RGB_565_MODE;
                        cam_width = 40;  // DEFAULT_WIDTH;
                        cam_heigth = 40; // DEFAULT_HEIGHT;
                        cam_size = cam_width * cam_heigth * 2;
                    ...
                    e_poxxxx_init_cam();
                    if (cam_x1 == -1)
                    { // user did not specify: take default
